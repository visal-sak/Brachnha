...
export default function MathCardOperation() {
  let grade = typeof window !== 'undefined' ? localStorage.getItem("grade") : null;
  const {data:calculate,isLoading,error}=useGetRequestGameByCodeQuery(`GOTM${grade}`)
  if (isLoading) {
            ...
            <div class="w-full block h-full">
              ...
              <div class="w-full">
                ...
                <div class="flex flex-wrap justify-between items-center mt-4 pt-5">
                  <div class="text-base mr-2 py-1.5 px-4 text-gray-700">
                    ហ្គេម
                  </div>
                  <Link href={`/games/${calculates.code}`}>
