...
import { useGetRequestAllGameCompareByCodeQuery } from "src/store/features/games/compare/requestCompareGame";
...
export default function CompareList() {
  ...
  const {
    data: games
  } = useGetRequestAllGameCompareByCodeQuery("GCP");
  //is disable account
        ...
        case "game":
          deleteGame(selectedRecord.uuid)
            .then(() => {
              ...
              setShowModal(false);
              toast.error("ប្រភេទ game compare ត្រូវបានលុបដោយជោគជ័យ!", {
                theme: "colored",
                icon: "🗑️",
                autoClose: 1000,
  ...
  const columnGameTypes = [
    ...
    },
    {
      ...
      cell : (row) => (
        row?.data?.answers.map((item,index)=>(
          <div className="ml-3 w-96 grid grid-rows-3">
            <div>
    ...
    name: "Disable",
      cell: (row) => (
        <div>
          <label className="relative ml-40 inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              value=""
