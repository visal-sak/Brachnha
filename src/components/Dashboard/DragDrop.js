...
import { useGetRequestAllGameDragDropByCodeQuery } from "src/store/features/games/dragdrop/requestDragDropGame";
...
export default function DragDropList() {
  ...
  const {
    data: games
  } = useGetRequestAllGameDragDropByCodeQuery("GDD");
        ...
        case "game":
          deleteGame(selectedRecord.uuid)
            .then(() => {
              ...
              setShowModal(false);
              toast.error("បានលុប game ប្រភេទ drag drop ដោយជោគជ័យ!!", {
                theme: "colored",
                icon: "🗑️",
                autoClose: 1000,
  ...
  const columnGameTypes = [
    ...
    },
    {
      name: "រូបភាពចម្លើយ",
      cell : (row) => (
        row?.data?.question?.dragdrop.map((item,index)=>(
          <div className="w-50 grid grid-rows-2">
            <div>
             <img className="w-14 p-1 " src={item.path}/> 
            </div>
          <div>
            <p className="mt-2 w-20 ">{item?.codeGame}</p>
          </div>
      ...
      cell : (row) => (
        row?.data?.answers.map((item,index)=>(
          <div className="ml-3 w-96 grid grid-rows-2">
            <div>
              <p className="w-36 p-1 " >{item?.word}</p>
            </div>
          <div>
            <p className="mt-2 w-20 ">{item?.codeGame}</p>
          </div>
    ...
    },
    {
      ...
      cell: (row) => (
        <div>
          <label className="relative left-56 inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              value=""
