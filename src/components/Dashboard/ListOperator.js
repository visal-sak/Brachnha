...
import { useGetRequestAllGameCalculateByCodeQuery } from "src/store/features/games/calculate/requestCalculateGame";
...
export default function OperatorList() {
  ...
  const {
    data: games
  } = useGetRequestAllGameCalculateByCodeQuery("GOT");
  //is disable account
        ...
        case "game":
          deleteGame(selectedRecord.uuid)
            .then(() => {
              ...
              setShowModal(false);
              toast.error("ប្រភេទ game operation ត្រូវបានលុបដោយជោគជ័យ!", {
                theme: "colored",
                icon: "🗑️",
                autoClose: 1000,
  ...
  const columnGameTypes = [
    ...
    {
      name: "ចម្លើយត្រឹមត្រូវ",
      selector: (row) => row?.data?.question?.result,
    },
    {
      name: "ចំនួនលេខគណនា",
      selector: (row) => row?.data?.question?.calculates.map((item,index)=>(
        <div className="ml-3 w-50 grid grid-cols-2">
            <div>
              <p className="w-20 p-1 " >{item?.value}</p>
            </div>
            <div>
              <p className="w-20 p-1 " >{item?.valueKh}</p>
            </div>
          </div>
      ))
    },
